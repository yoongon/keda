		...
		for _, spec := range metricSpecs {
			...
			if modifiers.ArrayContainsElement(spec.External.Metric.Name, metricsArray) {
				...
				if !metricsFoundInCache {
					...
					if latency != -1 {
						metricscollector.RecordScalerLatency(scaledObjectNamespace, scaledObject.Name, scalerName, scalerIndex, metricName, true, float64(latency))
					}
				...
				if err != nil {
					...
					for _, metric := range metrics {
						...
						metricscollector.RecordScalerMetric(scaledObjectNamespace, scaledObjectName, scalerName, scalerIndex, metric.MetricName, true, metricValue)
					}
					matchingMetrics = append(matchingMetrics, metrics...)
				}
				metricscollector.RecordScalerError(scaledObjectNamespace, scaledObjectName, scalerName, scalerIndex, metricName, true, err)
			}
		}
	}
...
func (*scaleHandler) getScalerState(ctx context.Context, scaler scalers.Scaler, scalerIndex int, scalerConfig scalers.ScalerConfig,
	...
	for _, spec := range metricSpecs {
		...
		if latency != -1 {
			metricscollector.RecordScalerLatency(scaledObject.Namespace, scaledObject.Name, scalerName, scalerIndex, metricName, true, float64(latency))
		}
		...
		if err != nil {
			...
			for _, metric := range metrics {
				metricValue := metric.Value.AsApproximateFloat64()
				metricscollector.RecordScalerMetric(scaledObject.Namespace, scaledObject.Name, scalerName, scalerIndex, metric.MetricName, true, metricValue)
			}
			if !scaledObject.IsUsingModifiers() {
				if isMetricActive {
					if spec.External != nil {
				...
				metricscollector.RecordScalerActive(scaledObject.Namespace, scaledObject.Name, scalerName, scalerIndex, metricName, true, isMetricActive)
			}
		}
